...
function pushHistoryState(page, section)
{
	...
	if (isValidString(pushTitle))
	{
		document.title = pushTitle;
		pushTitle = null;
	}
		...
		$.ajax(
		{
			...
			success: function(result, status, xhr)
			{
				...
				$("#dropdown_sections").dropdown(
				{
					...
					{
						let title = "{0} | {1} | RAVE".format(text, page);
...
function handleLoad(link, push, titleOverwrite = true)
{
...
{
	handleLoad(event.target.location, false, false);
});
